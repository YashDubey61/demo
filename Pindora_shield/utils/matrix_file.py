...
class MatrixPredictor:
    ...
    def predict_ic50(self, smiles: str) -> Dict[str, float]:
        ...
        log_ic50 = self._to_scalar(self.ic50_bundle["model"].predict(X)[0])
        return {
    ...
    def predict_association(self, smiles: str) -> Dict[str, float]:
        X = self.smiles_to_fp(smiles)
        log_assoc = self._to_scalar(self.assoc_bundle["model"].predict(X)[0])
        return {
    ...
    def predict_phase(self, smiles: str) -> int:
        X = self.smiles_to_fp(smiles)
        return int(self._to_scalar(self.phase_bundle["model"].predict(X)[0]))

    def predict_target(self, smiles: str) -> str:
        X = self.smiles_to_fp(smiles)
        idx = int(self._to_scalar(self.target_bundle["model"].predict(X)[0]))
    ...
    def predict_all(self, smiles: str) -> Dict[str, Any]:
        ...
        log_ic50 = self._to_scalar(self.ic50_bundle["model"].predict(X)[0])
        log_assoc = self._to_scalar(self.assoc_bundle["model"].predict(X)[0])
        phase = self._to_scalar(self.phase_bundle["model"].predict(X)[0])
        target_idx = self._to_scalar(self.target_bundle["model"].predict(X)[0])
